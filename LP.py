...
def segment_chars(plate_img, fixed_width):
    ...
    # resize LP region to a canoncial size
    plate_img = imutils.resize(plate_img, width=fixed_width)
    thresh = imutils.resize(thresh, width=fixed_width)
    bgr_thresh = cv2.cvtColor(thresh, cv2.COLOR_GRAY2BGR)
    # perform a connected components analysis and init mask to store the locations of the character candidates
    ...
    for label in np.unique(labels):
        ...
        if len(cnts) > 0:
            ...
            keppHeight = heightRatio > 0.5 and heightRatio < 0.95

            if keepAspectRatio and keepSolidity and keppHeight and boxW > 14:
...
class PlateFinder:
    ...
    def check_plate(self, input_img, contour):
        ...
        if self.validateRatio(min_rect):
            ...
            if plateFound:
                char_on_plate = self.find_char_on_plate(after_clean_plate_img)

                if(char_on_plate is not None and len(char_on_plate) == 8):
                    x1, y1, w1, h1 = coordinates
                    coordinates = x1 + x, y1 + y
                    after_check_plate_img = after_clean_plate_img
                    return after_check_plate_img, char_on_plate, coordinates
    ...
    def find_possible_plates(self, input_img):
        ...
        for cnts in possible_plate_contours:
            plate, char_on_plate, coordinates = self.check_plate(input_img, cnts)

            if plate is not None:
                plates.append(plate)
                self.char_on_plate.append(char_on_plate)
    ...
    def find_char_on_plate(self, plate):
        charFound = segment_chars(plate, 400)
        if charFound:
            return charFound
